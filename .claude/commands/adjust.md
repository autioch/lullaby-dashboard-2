...
`/implement`) but it does **not** edit the contract docs: the spec, plan, and summary are
**frozen** here. They will lag the code afterward; `/reconcile` re-syncs them later.
...
1. **Locate the feature & read its frozen artifacts.** Resolve `$ARGUMENTS` to a feature in
   `docs/features/` and read its **spec** (`NN-name.md`), **plan** (`NN-name.plan.md`), and
   **summary** (`NN-name.summary.md`) **in full** — these are the canonical record of what
   shipped and are **read-only for this command**. If `$ARGUMENTS` is empty or ambiguous, list
   the features that have a `NN-name.summary.md` (i.e. have been implemented, even partially —
   the spec `Status` may still read `agreed` if close-out didn't flip it) and ask which. If a
   summary is missing or partial, or the artifacts
...
2. **Ground yourself in the real code, narrowly.** From WORKFLOW's grounding reads and the
   summary's **Added** / **Changed** + the spec's **Impact on the codebase**, open the actual
...
4. **Implement each accepted request in order.** For each, like an `/implement` step:
   ...
   3. **Sync non-frozen docs in the same change** per the dev guide's **Keeping docs in sync**
      map (e.g. `docs/07`, README, the dev guide) — but **never** the spec, plan, or summary.
...
6. **Write the adjustments artifact.** Copy
   [`docs/features/_ADJUSTMENTS_TEMPLATE.md`](../../docs/features/_ADJUSTMENTS_TEMPLATE.md) to
   `docs/features/NN-name.adjustments-N.md` (**same `NN` and name as the spec**, `N` = this
   round). Fill it from this run: per request — **source/who**, the **request**, **why**
   ...
   **Verification** (gate + acceptance + which review skills ran) and **Commits**
   (`<sha> — <subject>`). End with the **Drift** note: the spec/plan/summary were intentionally
...
- **Frozen contracts:** never edit the spec (`NN-name.md`), plan (`NN-name.plan.md`), or summary
  (`NN-name.summary.md`) in this command — they are read-only inputs. Resulting drift is expected
  and is resolved later by `/reconcile`, not here.
