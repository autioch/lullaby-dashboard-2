...
norms, common sense, UX standards, and **technical viability on the SmartTV Chrome 87 floor**,
**converge with the user on the final shape**, then record a **verdict**. Investigation-only — **no
app code, no spec content**. The output is the spike artifact and a recommendation. This is the
optional pre-`/spec` lane: a place to reshape or kill an idea cheaply before committing to a
contract.
...
3. **Investigate and elicit — a dialogue, not a one-shot.** Use `AskUserQuestion` (focused,
   multiple-choice, recommended option first) to clarify, expand, or **challenge** the scope, going
   only as deep as needed to de-risk the core uncertainty. Vet the idea across four lenses:
   ...
   Re-ask as findings shift the picture; name what stays uncertain rather than over-investigating.

4. **Form a draft verdict.** One of `viable` · `viable-with-changes` · `not-viable` ·
   `needs-more-investigation`, with the options & trade-offs and a recommended approach behind it.
   This is a **proposal to discuss with the user**, not the final word.

5. **Converge with the user on the final shape.** Play the draft verdict back to the user — the
   options & trade-offs, the recommended approach, and, most importantly, **how the idea has changed
   from what they first assumed** (reshaped scope, a different approach, or a "no"). Then **loop**:
   take their pushback, adjust the findings / scope / verdict, and play it back again, until **you
   both agree the shape is final**. A spike often lands somewhere different from the opening idea —
   reaching that shared, settled shape is the point of this step. **Do not write the artifact on a
   unilateral verdict** — proceed only once the user agrees (or explicitly tells you to record it
   as-is).

6. **Write the spike — always, even for standalone brainstorming.** Copy
   ...
   `<short-name>` — reuse it if this graduates to `/spec`). Fill every section per the template **to
...
- **Agree before recording.** The verdict and final shape are converged with the user (step 5), not
  declared — especially when the spike reshapes the original idea. Record the agreed shape, not your
  first draft.
