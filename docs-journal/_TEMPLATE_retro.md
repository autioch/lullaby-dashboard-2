# <Feature name> — Retrospective

> **Artifact:** `NN_retro_<short-name>.md` · **Roles:** Product Owner (lead) · all roles weigh in
> **Status:** terminal record (no lifecycle)
> **Owner:** <name> · **Date:** YYYY-MM-DD
> **Related:** [spec](NN_spec_<short-name>.md) · [plan](NN_plan_<short-name>.md) · [implement](NN_implement_<short-name>.md) · [adjust](NN_adjust_<short-name>-r1.md)

Review of the **whole** development iteration — spec → plan → implement → adjust —
led by the **Product Owner** with every role weighing in (the **Tech Lead** on plan fidelity and
churn, the **Developer** on what bit during the build, Design on the UX calls). The wrap-up of the
lifecycle: read it, then decide the next move. Terse and blunt; every claim traces to an artifact
or commit.

## Signals

The few facts the verdict rests on. Fill what applies; cut the rest.

- **Plan fidelity:** did the build follow the plan, or diverge? (cite the implementation record's
  _Skipped / deferred_, any in-flight spec/plan edits)
- **Churn:** <N> adjustment round(s) — genuine new requirements vs. rework the spec/plan should
  have caught.
- **Loose ends:** deferred items, real-TV-only checks, follow-ups carried forward.

## Doc reconciliation

The repo-wide durable-doc audit against the current code (per the doc-sync map). List the docs
brought back in sync, or state "no drift — durable docs already current."

- ...

## What went well

Blunt. Each bullet points at evidence.

- ...

## What to improve

Blunt. Each bullet points at evidence — what to do differently next iteration, and why.

- ...

## Bottom line

One or two lines: the product owner's verdict on this iteration.

## Team feedback

Each role's own read, captured from the retro Q&A — what worked, what frustrated, what they'd
change about process or result. Their words, not paraphrased into agreement. Attribute by role
(Product Owner / Tech Lead / Developer / Design) where it helps.

- ...

## Suggested next actions

Concrete, prioritized — each tied to a problem above. Suggestions only; the owner decides.
Candidates: a follow-up `/spec` for descoped scope · a process / tooling tweak · tech-debt to
schedule · nothing, ship as is. The ones the owner carries forward are **also filed into
`docs/06_roadmap.md`** (title · one-sentence why · `(retro [NN])` source); this section keeps the
record.

- ...

## Decision

Left for the owner to record what they will actually do next.

- ...
