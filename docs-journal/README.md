...
- `NN_<command>_<short-name>.md` — artifacts for one feature (zero-padded `NN`, kebab-case
  ...
  Pre-spec **spike** artifacts live in the sibling [`docs-spikes/`](../docs-spikes/) folder, not here —
  investigation-only `/spike` records on their own local `NN` sequence (many never become features); a
  graduating spike reuses its `<short-name>` for the feature.
